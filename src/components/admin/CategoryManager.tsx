...
export const getCategoryGroupColor = (groupKey: string): string => {
  const mainCategories = Object.keys(MAIN_CATEGORY_MAPPING) as MainCategoryKey[];
  if (mainCategories.includes(groupKey as MainCategoryKey)) {
    return CATEGORY_GROUP_COLORS[groupKey as MainCategoryKey];
  }
  return '#95a5a6';
};
...
export class CategoryManager {
  ...
  setMainCategory(mainCategory: string): { success: boolean; message?: string } {
    const validCategories = this.getMainCategories();
    if (!validCategories.includes(mainCategory as MainCategoryKey)) {
  ...
  getCurrentMainCategory(): MainCategoryKey | null {
